...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.reecorp.com/danh-muc-bao-cao/bao-cao-tai-chinh/', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('h3.vii-report-item__title').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.REE);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.REE);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của REE ::: ${name}`);
        })
      );
