...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.pecc1.com.vn/c3/pages-t/Bao-cao-tai-chinh-5-453.aspx', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    const financialDiv = $('.timeline-v2');

    financialDiv.find('a').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.TV1);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.TV1);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của TV1 ::: ${name}`);
        })
      );
