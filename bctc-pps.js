...
async function fetchAndExtractData() {
  try {
    ...
    const response = await axios.get(`http://www.pvps.vn/tin-tuc-co-dong`, {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.PPS);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.PPS);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của PPS ::: ${name}`);
        })
      );
