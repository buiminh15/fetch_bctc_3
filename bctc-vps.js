...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://vipesco.com.vn/quan-he-co-dong.html', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    const financialDiv = $('[data-report="financial"]');

    financialDiv.find('p').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.VPS);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.VPS);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của VPS ::: ${name}`);
        })
      );
