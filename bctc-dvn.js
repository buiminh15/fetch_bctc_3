...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://vinapharm.com.vn/vn/bao-cao-tai-chinh', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.DVN);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.DVN);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của DVN ::: ${name}`);
        })
      );
