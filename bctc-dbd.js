...
async function fetchAndExtractData() {
  try {
    const response = await axios.get(`${CAFEF_API}${COMPANIES.DBD}`, {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.DBD);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.DBD);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của DBD ::: ${name}`);
        })
      );
