...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://vinafor.com.vn/bao-cao-ket-qua-kinh-doanh.html', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.VIF);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.VIF);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của VIF ::: ${name}`);
        })
      );
