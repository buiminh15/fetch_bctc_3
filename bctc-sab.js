...
async function fetchAndExtractData() {
  try {
    ...
    const response = await axios.get(`https://sabeco.com.vn/co-dong/bao-cao-tai-chinh/${currentYear}-2`, {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.SAB);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.SAB);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của SAB ::: ${name}`);
        })
      );
